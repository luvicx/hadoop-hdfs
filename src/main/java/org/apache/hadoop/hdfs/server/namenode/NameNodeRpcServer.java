...
class NameNodeRpcServer implements NamenodeProtocols {
  ...
  @Override // ClientProtocol
  //wcx reads 这边读过了，读到这里已经快要受不了了，从client到这里调用了4层了。。。
  public LocatedBlocks getBlockLocations(String src, 
                                          long offset, 
                                          long length) 
      throws IOException {
	//wcx metrics有点意思，又是一个记录统计数据的
    metrics.incrGetBlockLocations();
    
    //wcx 在这里尝试记录一下统计数据
    //the modification is useless, the hdfs_file_status is a new one, cannot addReadCount
    //HdfsFileStatus hdfs_file_status = namesystem.getFileInfo(src, false);
    //hdfs_file_status.addRead_counts();
    //LOG.info("[[wcx added]] read count added to"+hdfs_file_status.getRead_counts()+"");
    //以上是自己加的
