/**
 * wcx newed java class, in order to change the blockPlacementPolicy
 * 
 * 
...
public class BlockPlacementPolicyWithPopularity extends BlockPlacementPolicy {
  ...
  //构造函数，似乎没有什么用
  protected BlockPlacementPolicyWithPopularity() {
  }
  ...
  //重要的部分，选择放置的位置，这个好像就是调用来调用去的，先不改了
  @Override
  ...
  private DatanodeStorageInfo[] chooseTarget(int numOfReplicas,
                                    ...
                                    long blocksize,
                                    final BlockStoragePolicy storagePolicy) {
	//副本数为0，或者没有节点可以去存放，返回empty
    if (numOfReplicas == 0 || clusterMap.getNumOfLeaves()==0) {
      return DatanodeStorageInfo.EMPTY_ARRAY;
    }
    
    //excludedNodes是指不考虑用来存放的节点，即使为null，也给申请一个HashSet空间
  ...
  private Node chooseTarget(int numOfReplicas,
    ...
    try {
      ...
      //从chooserandom改成这个了
      chooseLeastWeightedNode(numOfReplicas, NodeBase.ROOT, excludedNodes, blocksize,
  ...
  protected DatanodeStorageInfo chooseLocalStorage(Node localMachine,
    ...
    if (localMachine == null) {
      ...
      //    maxNodesPerRack, results, avoidStaleNodes, storageTypes);
      return chooseLeastWeightedNode(NodeBase.ROOT, excludedNodes, blocksize,
              maxNodesPerRack, results, avoidStaleNodes, storageTypes);
    }
  ...
  protected DatanodeStorageInfo chooseLocalRack(Node localMachine,
    ...
    try {
      ...
      return chooseLeastWeightedNode(localRack, excludedNodes,
             blocksize, maxNodesPerRack, results, avoidStaleNodes, storageTypes);
    } catch (NotEnoughReplicasException e) {
  ...
  private DatanodeStorageInfo chooseFromNextRack(Node next,
    ...
    try {
      ...
      //    results, avoidStaleNodes, storageTypes);
      return chooseLeastWeightedNode(nextRack, excludedNodes, blocksize, maxNodesPerRack,
              results, avoidStaleNodes, storageTypes);
    } catch(NotEnoughReplicasException e) {
      if (LOG.isDebugEnabled()) {
  ...
  protected void chooseRemoteRack(int numOfReplicas,
    ...
    try {
    	//wcx 待修改：不能Random的选择，要选择负载最轻的
      //chooseRandom(numOfReplicas, "~" + localMachine.getNetworkLocation(),
      //    excludedNodes, blocksize, maxReplicasPerRack, results,
      //    avoidStaleNodes, storageTypes);
    	chooseLeastWeightedNode(numOfReplicas, "~" + localMachine.getNetworkLocation(),
    		 excludedNodes, blocksize, maxReplicasPerRack, results,
    		 avoidStaleNodes, storageTypes);
      ...
      chooseLeastWeightedNode(numOfReplicas-(results.size()-oldNumOfReplicas),
                   localMachine.getNetworkLocation(), excludedNodes, blocksize, 
                   maxReplicasPerRack, results, avoidStaleNodes, storageTypes);
    }
  }
  ...
  /**
   * wcx writes choose one target from the given scope,
   * the one is the least weighted on in the scope
   * 
   */
  protected DatanodeStorageInfo chooseLeastWeightedNode(String scope,
	      Set<Node> excludedNodes,
	      long blocksize,
	      int maxNodesPerRack,
	      List<DatanodeStorageInfo> results,
	      ...
	      EnumMap<StorageType, Integer> storageTypes)
	          throws NotEnoughReplicasException {
	    return chooseLeastWeightedNode(1, scope, excludedNodes, blocksize, maxNodesPerRack,
	        results, avoidStaleNodes, storageTypes);
	  }
  
  protected DatanodeStorageInfo chooseLeastWeightedNode(int numOfReplicas,
          String scope,
          Set<Node> excludedNodes,
          long blocksize,
          int maxNodesPerRack,
  ...
  int numOfAvailableNodes = clusterMap.countNumOfAvailableNodes(
	    ...
	    while(numOfReplicas > 0 && numOfAvailableNodes > 0) {
	    	//wcx 待修改，不应该是chooseRandom，需要自己写一个新函数
