...
//wcx 应该就是记录file级别统计信息的类，需要添加读写频率等信息~
//wcx 但是里面都是final变量，在调用时不知道会不会出问题
...
public class HdfsFileStatus {
  ...
  //wcx add 添加新的成员，读次数和写次数,int 在实验中应该够用了
  private int read_counts;
  private int write_counts;
  ...
  public HdfsFileStatus(long length, boolean isdir, int block_replication,
    ...
    //wcx add 在构造函数中也需要添加相应的初始值
    this.read_counts = 0;
    this.write_counts = 1;//既然文件存在了，应该只有一次吧。。。
  }
  ...
  //wcx add getter and setter
  /**
   * Getter and Setter for the read_counts and set_counts
   * @return
   */
  ...
  public void setRead_counts(int read_counts) {
	this.read_counts = read_counts;
	}
  
  public void addRead_counts() {
		this.read_counts++;
		}

  public int getWrite_counts() {
	return write_counts;
  }

  public void setWrite_counts(int write_counts) {
	this.write_counts = write_counts;
  }
  
  public void addWrite_counts() {
		this.write_counts++;
	  }
  
  //wcx 以上是我自己加的，可能线程不安全
